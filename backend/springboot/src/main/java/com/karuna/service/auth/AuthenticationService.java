...
public class AuthenticationService {
	...
	@Transactional
	public AuthResponseDTO googleLogin(GoogleAuthRequestDTO request, ClientContext clientContext) {
		String idToken = request.getIdToken();

		// 1. Verify token with Google's API endpoint
		...
		try {
			...
			googleClaims = (Map<String, Object>) response.getBody();
		} catch (Exception e) {
			throw new BadCredentialsException("Invalid Google token or connection failure: " + e.getMessage());
		}

		String email = (String) googleClaims.get("email");
		String name = (String) googleClaims.get("name");
		if (email == null) {
			throw new BadCredentialsException("Google ID token does not contain email claim");
		}

		// 2. Load or register user
		...
		if (userOptional.isPresent()) {
			...
			user.setPasswordHash(passwordService.encode(java.util.UUID.randomUUID().toString())); // Random password hash
			user.setRole(StringUtils.hasText(request.getRole()) ? request.getRole() : "CITIZEN");
			user.setEmailVerified(true);
			user.setActive(true);
			userRepository.save(user);
